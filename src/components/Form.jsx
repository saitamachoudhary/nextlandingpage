    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-20">
      <div className="max-w-md w-full flex flex-col p-6 rounded-md bg-white text-black">
        <div className="text-2xl font-bold mb-2 text-[#1e0e4b] text-center">
          Welcome back to <span className="text-black">App</span>
        </div>
        ...
        >
          {({ handleSubmit }) => (
            <Form className="flex flex-col gap-3" onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                className="bg-black w-max m-auto px-6 py-2 rounded text-white text-sm font-normal"
              >
                Submit
              </button>
            </Form>
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 text-black">
          <div className="bg-white p-6 rounded-md max-w-sm w-full">
            <h2 className="text-2xl font-bold text-center text-black mb-4">Form Submitted</h2>
            <div className="text-center">
              <p><strong>Email:</strong> {formData?.email}</p>
              <p><strong>Password:</strong> {formData?.password}</p>
              <button
                className="mt-4 bg-black px-4 py-2 text-white rounded"
                onClick={() =>{
                    CloseLoginmodal();
                    setShowPopup(false);
                }}
